...
#Parameters:
#prarameters
#K = population carrying capacity...  numeric (vector for multiple populations)
...
PopMod = function(t,A,K,P,B,S,f,H,H_delay,E,Ep,R,Target_ages,H_skew,H_d,P_D,O_D,W_D,M){
  ...
  } #calculate the effect of carrying capacity at t based on population size at t-1
  ...
  entangle = function(PI){
        # PI = vector of abundance per age class 
        # entangle is called once per county
        
        # total abundance before applying bycatch
        init_sum = sum(PI)
        
        
        # partion total bycatch in age classes according to Ep
        E <- Ep/sum(Ep) * init_sum*E
        
        tot <- 0
        E_unallocated <- 0
        printf("expected total bycatch=%.03f\n", sum(E))
        for(i in 1:length(PI)) {
              newN <- PI[i] - E[i]
              remainder <- 0
              
              # if bycatch would reduce abundance of age cohort
              # to less than 0, 
              if (newN < 0) {
                    remainder <- abs(newN)
                    newN <- 0
              ...
              printf("Age = %d, N = %.02f - %.02f = %.02f, remainder = %.02f\n", 
                     i, PI[i], E[i], newN, remainder)
              
              PI[i] <- newN
              
              tot <- tot + E[i] - remainder
              E_unallocated <- E_unallocated + remainder
        }
        
        # apply the remaining bycatch to the population as a proportion
        prop <- E_unallocated/sum(PI) 
        newN <- (1-prop) * PI
        printf("Total bycatch = %.03f (by age = %.03f, by perc = %.03f, remainder = %.03f)\n", 
               tot +  (sum(PI) - sum(newN)), tot, sum(PI) - sum(newN), E_unallocated)
        PI <- newN
